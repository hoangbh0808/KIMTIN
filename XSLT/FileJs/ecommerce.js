...
$(document).ready(function () {
	...
	$("body").on("click", ".ajaxresponseviewmore", function (e) {
		...
		searchData($(this));
		$("html,body").animate({
				scrollTop: $(".ajaxresponsewrap").offset().top -
					$("header").outerHeight(),
			},
			"slow"
		);
	...
	function searchData(obj) {
		...
		if ($(".ajaxfilterlink.active").length) {
			for (
				var i = 0; i <
				$(".product-filter-item.field .ajaxfilterlink.active").length; i++
			) {
				params +=
					sepa +
	...
	function ProcessAjax(pageurl) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				$(".product-filter-item")
					.not(".product-filter-price")
					.each(function (index, element) {
						var triggerBtn = $(element).find(
        ...
        */
		...
		$.ajax({
			...
			success: function (data) {
				...
				$(".ajaxresponse .btn-center").append(
					$(data).find(".ajaxresponse-pager-wrap").html()
				); //add new
			},
		});
...
var AjaxCart = {
	...
	topwishlistselector: "",
	flyoutcartselector: ".list-cart-product",
	flyoutcarttotalselector: ".text .cart-flyout-total",
	cartpopup: null,
	...
	addproducttocart_catalog: function (button) {
		...
		var quantity = 1;
		var inputQuantity = $(button).parent().find("input[name='addtocart_" + $(button).attr("data-productid") + ".EnteredQuantityCatalog']");
	...
	success_desktop: function (response) {
		...
		if (response.updateflyoutcartsectionhtml) {
			...
			if ($(AjaxCart.flyoutcartselector)) {
				...
				//console.log($(AjaxCart.flyoutcartselector).html());
				//console.log($(response.updateflyoutcartsectionhtml).html());
				...
				$(AjaxCart.flyoutcarttotalselector).replaceWith(
					$(response.updateflyoutcartsectionhtml).find(
						AjaxCart.flyoutcarttotalselector
					)
				);				
				if (checkoutUrl)
					$(AjaxCart.flyoutcartselector)
