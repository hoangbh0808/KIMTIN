...
var AjaxCart = {
	...
	addproducttocart_quick: function (button){
		...
		$.ajax({
			...
			type: "post",
			success: function (response) {
				let carturl = $(".shopping-cart-wrapper .btn-center .btn-primary").attr('href');
	...
	selectproductoption: function (button) {
        ...
        $.each($(button).parents('.options-wrap').find('.product-options'), function () {
				...
				if (response.success == true) {
					if (response.productName){
						...
						if (response.editLink){
							...
							pName += "<a title='Sửa' class='edit-link' href='" + response.editLink + "'> <i class='fa fa-pencil'></i></a>"
						}
					...
					if (response.childProductDetail) {
						if ($(response.childProductDetail).find(".wrap-img-product") != undefined)
							$('.wrap-img-product').replaceWith($(response.childProductDetail).find(".wrap-img-product"))
					}
					ReloadProductDetailSlider();
					/*
					if (response.productCode)
...
var AjaxCheckout = {
        ...
        if($("input[provider=15]").length > 0)
            $("input[provider=15]").each(function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success == true) {
                            $(currentShipping).parents(".select-group").find("select").html(response.data);
                            $(currentShipping).parents(".select-group").find("select").trigger("change");
                        }
                    }
                });
            });
	...
	saveorder: function (savetodb, redirect) {
		...
		var data = $("#aspnetForm").serializeArray();
		let sameBilling = "off";
		if($(".user-info.clearfix").is(':visible'))
			sameBilling = "on";
		data.push({
