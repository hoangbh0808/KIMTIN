...
/*==================== Check width home-choose and count number ====================*/
function checkWidthHomeChoose() {
	...
	count.each(function (index) {
		let width = Number($(this).css("width").slice(0, -2)) + "px";
		console.log(width);
		$(this).css("min-width", width);
		// count
		var value = Number($(this).attr("data-count"));
		var count123 = new CountUp(count[index], 0, value, 0, 5, countOptions);
		count123.start();
	});
...
function CheckFilterOrder() {
	$(".filter-wrap").each(function () {
		let count = $(this).find(".select-options li").length;
		if (count == 4) {
...
function CheckTable(e) {
	$('.content-product table').wrap('<div class="wrap-table-responsive"></div>')
}
/*==================== LOAD FUNCTION ====================*/
$(document).ready(function () {
	...
	InsertMobileAccount();
	checkActiveFilter();
	var waypoints = $("#home-choose-id").waypoint(
		function (direction) {
