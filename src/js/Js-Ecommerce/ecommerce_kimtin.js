...
var AjaxCart = {
	...
	addproducttocart_popup: function (button) {
		...
		$.ajax({
			...
			success: function (response){
				$.fancybox.close();
				setTimeout(function(){ AjaxCart.success_desktop(response); }, 1000);				
			},
			complete: this.resetLoadWaiting,
			error: this.ajaxFailure
		});
            ...
            success: function (response) {
                if (response.success == true) {
					...
					if (response.childProductDetail) {
						...
						if ($(response.childProductDetail).find(".wrap-slide-detail") != undefined)
							$('.wrap-slide-detail').html($(response.childProductDetail).find(".wrap-slide-detail"))						
					}
