...
$(document).ready(function () {
    ...
    $('body').on('click', '.product-option', function () {
		if ($("body").hasClass("product-detail-page")) {
			var btn = $(this);
			var input = btn.parent().parent().find('input[type="hidden"]');
			if (input)
			{
				input.attr("value", btn.attr("data-id"))
				btn.parent().find('.product-option').removeClass('active');
				btn.addClass('active');
			}
		}	
    });	
...
var AjaxCart = {
    ...
    effecttocart: true,
    topcartselector: '.cart-number .item-quantity',
    topwishlistselector: '',
    flyoutcartselector: '.cart-header',
    ...
    addproducttowishlist: function (button) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    $(button).find("em").attr("class", "mdi mdi-heart");
                }
                else {
                    $(button).find("em").attr("class", "mdi mdi-heart-outline");
                }
            },
            complete: this.resetLoadWaiting,
            error: this.ajaxFailure
    ...
    applycard: function () {
        ...
        var data = {
            ...
            'ShippingAddress_FirstName': $('#ShippingAddress_FirstName').val(),
            'ShippingAddress_Phone': $('#ShippingAddress_Phone').val(),
            'ShippingAddress_Email': $('#ShippingAddress_Email').val(),
            'ShippingAddress_Province': $('#ShippingAddress_Province').val(),
            'ShippingAddress_District': $('#ShippingAddress_District').val(),
            'ShippingAddress_Address': $('#ShippingAddress_Address').val(),
            'Invoice_CompanyTaxCode': $('#Invoice_CompanyTaxCode').val(),
            'Invoice_CompanyName': $('#Invoice_CompanyName').val(),
            'Invoice_CompanyAddress': $('#Invoice_CompanyAddress').val()
        };
        $.ajax({
            cache: false,
            url: urladd,
            ...
            success: function (response) {
                if (response.success == false)
                    $('.card-apply-error').text(response.message);
                else {
                    AjaxCheckout.saveorder(false, '');
                    location.reload();
                    //setLocation(response.redirect);
    ...
    success_desktop: function (response) {
        ...
        if (response.updateflyoutcartsectionhtml) {
            ...
            $(AjaxCart.flyoutcartselector).replaceWith($(response.updateflyoutcartsectionhtml));
            $(AjaxCart.cartpopup).addClass('open');
            if (checkoutUrl != null)
...
var AjaxCheckout = {
    ...
    getshippingtotal: function () {
		...
		var urladd = siteRoot + "/Product/Services/CheckoutService.aspx";
		var provinceGuid = $("select[name=Address_Province]").val();
		var districtGuid = $("select[name=Address_District]").val();
		if($('#divShipping').is(':visible')){
			provinceGuid = $("select[name=ShippingAddress_Province]").val();
			districtGuid = $("select[name=ShippingAddress_District]").val();			
		}		
		var data = {
			'method': 'GetShippingTotal',
			'shippingMethodId': shippingMethodId, 
			'provinceGuid': provinceGuid,
...
$(document).ready(function () {
    ...
    $("body").on("click", '.ajax-promo-zone-link', function (e) {
        ...
        var obj = $(this);
        $('.ajax-promo-zone-link').removeClass('active');
        obj.addClass('active');

        getPromoProducts(obj.data('zoneid'), true);
        ...
        //    data: data,
        //    success: function (data) {
        //        $('.ajax-promo-filter-response').html(data.filter);
        //        $('.ajax-promo-products-response').html(data.data);
    ...
    $("body").on("click", '.ajax-promo-option-link', function (e) {
        e.preventDefault();

        var obj = $(this);
        var hasActive = obj.hasClass('active');
        obj.parents('.filter-item').find('.ajax-promo-option-link').removeClass('active');

        if (!hasActive)
            obj.addClass('active');

        var zoneId = $('.ajax-promo-zone-link.active').data('zoneid');
        getPromoProducts(zoneId, false);

        return false;
    });

    $("body").on("change", '.ajax-promo-select', function (e) {
        e.preventDefault();
    ...
    function getPromoProducts(zoneId, updateFilter) {
        var pageSize = $('.ajax-promo-size').val();
        var sort = $('.ajax-promo-sort').val();
        ...
        var data = [];
        data.push({ name: 'method', value: 'GetPromoProducts' });
        data.push({ name: 'zoneid', value: zoneId });
        data.push({ name: 'sort', value: sort });
        data.push({ name: 'pagesize', value: pageSize });

        for (var i = 0; i < $(".filter-item").length; i++) {
            var link = $(".filter-item").eq(i).find('.ajax-promo-option-link.active').data('url');
            if (link) {
                var str = link.split('=');
                data.push({ name: str[0], value: str[1] });
            }
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (updateFilter)
                    $('.ajax-promo-filter-response').html(data.filter);
                $('.ajax-promo-products-response').html(data.data);
                $('.total-products').html(data.totalProducts);

                var paging = $('.ajax-promo-next-page');
                paging.attr('data-page', 1);
                paging.attr('data-zoneid', zoneId);

                if (data.nextPage > 0) {
                    paging.attr('data-page', data.nextPage);
                    paging.removeClass('hidden');
                } else {
                    paging.addClass('hidden');
					$('.promo-show-more').addClass('hidden');
				}
            }
        });
    }
    ...
    $("body").on("click", '.ajax-promo-next-page', function (e) {
        ...
        var obj = $(this);
		var pDiv = obj.parent('.promo-show-more');
        $.ajax({
            url: pageurl,
            type: 'post',
            data: data,
            success: function (data) {
                ...
                //$('.ajax-promo-products-response').html(data.data);
                $('.ajax-promo-products-response').append($(data.data));
                //$('.total-products').html(data.totalProducts);
    ...
    if ($('.ajax-promo-zone-link').length)
        getPromoProducts(-1, true);
